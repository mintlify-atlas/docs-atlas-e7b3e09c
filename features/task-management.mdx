...
Any user (authenticated or not) can open the task creation form at `/crear/`. On a successful `POST`, the new `Tarea` is saved and the browser is redirected to its detail page at `/detalle/<id>/`.
